 ...
 *
 * Run via Maven — single-port mode (swagger served from BFF, port 8081):
 ...
 *
 * Run via Maven — dev mode (swagger on port 3201, separate npm run dev process):
 *   mvn -pl bff-spring-projs/spring.oidc.bff verify -Pe2e \
 *       -Dokta.tenant.id=X -Dokta.oauth2.client-id=Y -Dokta.oauth2.client-secret=Z \
 *       -Dswaggeruiurl=http://localhost:3201 \
 *       -Dswagger.e2e.ui.url=http://localhost:3201
 *
 * Run directly (app already running, env vars set manually):
 *   cd bff-spring-projs/spring.oidc.bff/swagger-bff-e2e
 *   $env:SWAGGER_UI_URL="http://localhost:3201"   # omit for single-port mode
...
// ---------------------------------------------------------------------------
// Credentials / config
// SWAGGER_UI_URL — where the Swagger UI is served from
//   single-port mode : http://localhost:8081  (default, swagger inside BFF)
//   dev mode         : http://localhost:3201  (npm run dev -- --port 3201)
// BFF_URL           — the backend origin (always 8081)
// Both are injected as env vars by the Maven e2e profile.
// ---------------------------------------------------------------------------
const SWAGGER_UI_ORIGIN  = process.env.SWAGGER_UI_URL      ?? 'http://localhost:8081';
const BFF_ORIGIN         = process.env.BFF_URL             ?? 'http://localhost:8081';
const APP_URL            = `${SWAGGER_UI_ORIGIN}/swagger-ui/index.html`;
...
test('Swagger UI BFF login — redirect flow with Okta Verify TOTP', async ({ page }) => {
  ...
  //    Spring Security redirects to swaggeruiurl / /swagger-ui/index.html
