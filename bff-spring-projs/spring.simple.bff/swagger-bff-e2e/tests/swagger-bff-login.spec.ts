 ...
 *
 * Run via Maven — single-port mode (swagger served from BFF, port 8080):
 *   mvn -pl bff-spring-projs/spring.simple.bff verify -Pe2e
 *
 * Run via Maven — dev mode (swagger on port 3200, separate npm run dev process):
 *   mvn -pl bff-spring-projs/spring.simple.bff verify -Pe2e \
 *       -Dswaggeruiurl=http://localhost:3200 \
 *       -Dswagger.e2e.ui.url=http://localhost:3200
 *
 * Run directly (app already running, env vars set manually):
 *   cd bff-spring-projs/spring.simple.bff/swagger-bff-e2e
 *   $env:SWAGGER_UI_URL="http://localhost:3200"   # omit for single-port mode
...
//   single-port mode : http://localhost:8080  (default, swagger inside BFF)
//   dev mode         : http://localhost:3200  (npm run dev)
// BFF_URL           — the backend origin (always 8080)
...
// ---------------------------------------------------------------------------
const SWAGGER_UI_ORIGIN    = process.env.SWAGGER_UI_URL        ?? 'http://localhost:8080';
const BFF_ORIGIN           = process.env.BFF_URL               ?? 'http://localhost:8080';
...
test('Swagger UI BFF login — inline form flow (simple Spring Security)', async ({ page }) => {
  ...
  // ------------------------------------------------------------------
  await page.request.get(`${BFF_ORIGIN}/apilogout?source=swagger`);
