...
### 1. **HelloEndpointTest.java** (4 tests)
Tests the `/hello` endpoint which is publicly accessible but returns different content based on authentication status.

**Coverage:**
- ✅ Unauthenticated access (returns empty string)
- ✅ Authenticated as "user" with ROLE_myuser (returns "user")
- ✅ Authenticated as "admin" with ROLE_myadmin (returns "admin")
- ✅ Authenticated with custom user (returns custom username)

**Key Endpoint Behavior:** `permitAll()` - Public access, but response varies by authentication
...
|----------|----------------|-----------|------------|------------|----------------|---------------|
| `/hello` | ✅ 200 | ✅ 200 | ✅ 200 | ✅ 200 | N/A | Yes (permitAll) |
